...
for i=1:nSample
    ...
    if rand < 0.5
        angle = 2*pi*rand();
        radius = normrnd(3,0.05);
        XData1(i) = radius*cos(angle);
        YData1(i) = radius*sin(angle);
    else
        angle = 2*pi*rand();
        radius = normrnd(0,0.05);
        XData1(i) = radius*cos(angle);
        YData1(i) = radius*sin(angle);        
    end
    %
    angle = 2*pi*rand();
    radius = normrnd(1,0.05);
...
OData1 = [zeros(1,nSample)  ones(1,nSample)];
OData2 = [ones(1,nSample)  zeros(1,nSample)];
OData = [OData1;OData2];
end
